...
export const agentWakeupRequests = pgTable(
  ...
  (table) => ({
    ...
    // Makes the idempotency key enforceable instead of decorative for every
    // OTHER source that isn't one of the specific namespaces above: the
    // enqueue path checks for a live wake with the same key first, and this
    // index wins the race the check cannot see (two concurrent enqueues both
    // missing the other's uncommitted row).
